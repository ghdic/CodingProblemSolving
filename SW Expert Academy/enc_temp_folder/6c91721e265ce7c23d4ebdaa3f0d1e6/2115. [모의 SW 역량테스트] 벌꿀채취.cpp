...
struct honey {
	int y, x, val, dir;
	honey(int y = 0, int x = 0, int val = 0, int dir = 0) :y(y), x(x), val(val), dir(dir) {};
};
int n, m, c, result;
int map[10][10];
vector<honey> v;
...
bool isCollide(int a, int b) {
	...
	int ax = v[a].x + m * X[v[a].dir], bx = v[b].x + m * X[v[b].dir];
	if (ax > v[b].x || v[a].x < bx)return false;
	else return true;
}
...
void collect(int y, int x, int dir) {
	int h = map[y][x], cnt = 1, tx = x, val = map[y][x]*map[y][x]; // 꿀, 선택한 수
	
	while (true) {
		tx += X[dir];
		if (tx < 0 || tx >= n)break; // 경계벗어남
		if (h + map[y][tx] > c)break; // 최대 꿀 벗어남
		if (cnt == m)break; // 최대 고를 수 있는 꿀통 수
		++cnt;
		h += map[y][tx];
		val += map[y][tx] * map[y][tx];
	}
	v.push_back({ y, x, val, dir });
}
// 해당 셀 기준으로 연속되는 k개의 꿀칸을 채취했을때
// 왼쪽, 오른쪽으로 채취할 수 있는 값
void canCollect() {
	for (int i = 0; i < n; ++i) {
		for (int j = 0; j < n; ++j) {
