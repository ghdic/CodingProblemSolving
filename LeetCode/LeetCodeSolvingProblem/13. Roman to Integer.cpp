...
public:
    int romanToInt(string s) {
		...
		for (int i = 0; i < len - 1; i++) {
			char c = s[i];
			temp += m[c];
			if (c == s[i + 1]) { //문자가 같은 경우
				;
			}
			else if (m[c] > m[s[i + 1]]) { //문자가 더 큰 경우
				result += temp;
				temp = 0;
			}
			else if (m[c] < m[s[i + 1]]) { //문자가 더 작은 경우
				result -= temp;
				temp = 0;
			}
		}
...
Solution
1. 값을 임시변수에 더해준다.
2. 다음문자가 같은지 더 큰지, 더 작은지 확인한다.
3. 같을 경우 아무런처리 x, 더 큰 경우 result에 temp를 더해주고 초기화, 더 작은 경우 result에 temp를 빼주고 초기화
